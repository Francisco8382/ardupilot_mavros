...
namespace ardupilot_mavros {
    ...
    void Waypoint::CallbackPath(const ros::TimerEvent& event) {
      ...
      x = _param.DistanciaTrayectoria.x()*sin(W1*t) + _param.PosicionInicial.x();
      y = _param.DistanciaTrayectoria.y()*sin(W2*t) + _param.PosicionInicial.y();
      z = _param.DistanciaTrayectoria.z()*sin(W3*t) + _param.PosicionInicial.z();

      dot_x = _param.DistanciaTrayectoria.x()*W1*cos(W1*t);
      dot_y = _param.DistanciaTrayectoria.y()*W2*cos(W2*t);
      dot_z = _param.DistanciaTrayectoria.z()*W3*cos(W3*t);
      
      ddot_x = -_param.DistanciaTrayectoria.x()*pow(W1,2)*sin(W1*t);
      ddot_y = -_param.DistanciaTrayectoria.y()*pow(W2,2)*sin(W2*t);
      ddot_z = -_param.DistanciaTrayectoria.z()*pow(W3,2)*sin(W3*t);
      ...
      case Trayectoria:
        ...
        if (_param.Yaw_Enabled){
          yaw = atan2(dot_y,dot_x) + _param.Yaw_Offset;
        }
        else {
          yaw = _param.Yaw_Offset;
        }
